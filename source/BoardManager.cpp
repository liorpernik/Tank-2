...
void BoardManager::updateMap(unique_ptr<GameObject> obj, pair<int, int> new_pos)
{
    ...
    if (!game_map[new_pos.first][new_pos.second].empty() && game_map[new_pos.first][new_pos.second][0] == nullptr)
        game_map[new_pos.first][new_pos.second][0] = move(obj);
    else
        game_map[new_pos.first][new_pos.second].push_back(move(obj));
}
    ...
    for (size_t i = 0; i < fired_shells.size(); i++)
    {
        ...
        for (int step = 0; step < 2 && !shellDestroyed; step++)
        {
            ...
            if (game_map[oldPos.first][oldPos.second].size() > 1)
            {
                ...
                shellPtr = move(game_map[oldPos.first][oldPos.second][0]); // extractObjectFromMap(shell.get());
...
unique_ptr<GameObject> BoardManager::extractObjectFromMap(T *object)
{
    ...
    {
        if (it->get() == object)
        { // move ownership and remove from map, later gets back to map in updateMap
            ...
            if (cell.size() > 1)
            {
                ...
                if (cell.size() == 2)
                {
                    ...
                    for (auto &obj : cell)
                    {
                        ...
                        { // todo: check if destroyed or fix cleanup
                            hasShell = true;
                        }
